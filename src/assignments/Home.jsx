import React from "react";
import "./Home.css";
import { Link } from "react-router-dom";

function Home(){
    return(
        <div className="home">
        <nav className="box4">
        <h1>React Assignments</h1>
        <Link to="/ASG-01" >Assignment-01</Link>
        <Link to="/ASG-02" >Assignment-02</Link>
        <Link to="/ASG-03" >Assignment-03</Link>
        <Link to="/ASG-04" >Assignment-04</Link>
        <Link to="/ASG-05" >Assignment-05</Link>
        <Link to="/ASG-06" >Assignment-06</Link>
        <Link to="/ASG-07" >Assignment-07</Link>
        <Link to="/ASG-08" >Assignment-08</Link>
        <Link to="/ASG-09" >Assignment-09</Link>
        <Link to="/ASG-10" >Assignment-10</Link>
        <Link to="/ASG-11" >Assignment-11</Link>
        <Link to="/ASG-12" >Assignment-12</Link>
        <Link to="/ASG-13" >Assignment-13</Link>
        <Link to="/ASG-14" >Assignment-14</Link>
        <Link to="/ASG-15" >Assignment-15</Link>
        <Link to="/ASG-16" >Assignment-16</Link>
        <Link to="/ASG-17" >Assignment-17</Link>
        <Link to="/ASG-18" >Assignment-18</Link>
        <Link to="/ASG-19" >Assignment-19</Link>
        <Link to="/ASG-20" >Assignment-20</Link>
        <Link to="/ASG-21" >Assignment-21</Link>
        <Link to="/ASG-22" >Assignment-22</Link>
        <Link to="/ASG-23" >Assignment-23</Link>
        <Link to="/ASG-24" >Assignment-24</Link>
        <Link to="/ASG-25" >Assignment-25</Link>
        <Link to="/ASG-26" >Assignment-26</Link>
        <Link to="/ASG-27" >Assignment-27</Link>
        <Link to="/ASG-28" >Assignment-28</Link>
        <Link to="/ASG-29" >Assignment-29</Link>
        <Link to="/ASG-30" >Assignment-30</Link>
        <Link to="/ASG-31" >Assignment-31</Link>
        <Link to="/ASG-32" >Assignment-32</Link>
        <Link to="/ASG-33" >Assignment-33</Link>
        <Link to="/ASG-34" >Assignment-34</Link>
        <Link to="/ASG-35" >Assignment-35</Link>
        </nav>
        </div>
    )
}

export default Home